...
function Portfolio() {
  return (
    <section id="portfolio" className="relative overflow-hidden bg-[linear-gradient(180deg,#030306_0%,#070711_44%,#030306_100%)] px-4 py-24 md:px-6 md:py-44">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_18%_16%,rgba(123,223,229,.045),transparent_30%),radial-gradient(circle_at_84%_58%,rgba(216,111,189,.045),transparent_32%)]" />
      <div className="relative mx-auto max-w-7xl">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 lg:gap-5">
          {portfolio.map((item, i) => (
            <Reveal delay={Math.min(i * 0.06, 0.2)} key={item.title}>
              <a
                href="#contact"
                className="portfolio-card premium-card group relative block min-h-[440px] overflow-hidden rounded-[1.25rem] border border-white/10 bg-black text-left outline-none shadow-[0_18px_70px_rgba(0,0,0,.34)] md:min-h-[500px] md:rounded-[1.55rem]"
              ...
              >
                ...
                <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(0,0,0,.10)_0%,rgba(0,0,0,.28)_34%,rgba(0,0,0,.92)_100%)]" />
                <div className="absolute left-4 top-4 z-10 rounded-full border border-white/12 bg-black/42 px-3 py-2 text-[0.58rem] font-black uppercase tracking-[0.18em] text-white/60 backdrop-blur-md md:left-5 md:top-5">
                  {item.category}
                </div>
                <div className="absolute bottom-0 left-0 right-0 z-10 p-5 md:p-6">
                  <h3 className="font-display text-[clamp(2rem,8vw,3.35rem)] font-black uppercase leading-[0.9] tracking-[-0.06em] text-white md:text-[clamp(2rem,3.2vw,3.5rem)]">{item.title}</h3>
                  <div className="mt-5 space-y-3 text-sm leading-6 text-white/68 md:text-base md:leading-7">
                    <p><span className="font-black uppercase tracking-[0.12em] text-zephyr-cyan/84">What we create:</span> {item.create}</p>
                    <p><span className="font-black uppercase tracking-[0.12em] text-zephyr-magenta/82">Why it helps:</span> {item.why}</p>
                  </div>
                  <span className="premium-button mt-5 inline-flex min-h-12 items-center justify-center rounded-full border border-white/16 bg-white/[.06] px-5 py-3 text-[0.66rem] font-black uppercase tracking-[0.16em] text-white/84 backdrop-blur-sm group-hover:border-zephyr-cyan/36 group-hover:text-zephyr-cyan">
