...
export default function Home() {
  useEffect(() => {
    ...
    gsap.to('.gsap-hero-glow', { scale: 1.08, opacity: 0.38, duration: 3.8, repeat: -1, yoyo: true, ease: 'sine.inOut' });
    gsap.to('.gsap-float', { y: -7, duration: 4.2, repeat: -1, yoyo: true, ease: 'sine.inOut', stagger: 0.18 });
  ...
  return (
    <section id="hero" className="relative flex min-h-screen items-center justify-start overflow-hidden bg-black px-5 py-28 md:px-10">
      ...
      <div className="relative z-30 ml-0 max-w-4xl text-left md:ml-[4vw]">
        ...
        <h1 className="gsap-parallax text-glow font-display text-[clamp(2.45rem,8.5vw,6.8rem)] font-black uppercase leading-[0.82] tracking-[-0.09em]">
...
function Portfolio() {
  ...
  return <section id="portfolio" className="relative bg-gradient-to-b from-zephyr-black via-[#070713] to-zephyr-black px-4 py-28 md:px-6 md:py-36"><SectionTitle kicker="Portfolio" title="Fullscreen campaign worlds" copy="Tap any frame to open a cinematic preview." /><div className="mx-auto grid max-w-7xl gap-4 md:grid-cols-2 lg:grid-cols-3">{portfolio.map((item, i) => <Reveal delay={i * .04} key={item.title}><motion.button type="button" onClick={() => setActive(item)} whileHover={{ y: -6, scale: 1.006 }} whileTap={{ scale: 0.98 }} className="group relative min-h-[390px] w-full overflow-hidden rounded-[1.55rem] border border-white/10 bg-radial-cinema p-0 text-left shadow-[inset_0_0_80px_rgba(255,255,255,.03)] outline-none md:min-h-[470px]"><img src={item.image} alt="" className="absolute inset-0 h-full w-full object-cover opacity-46 grayscale-[35%] transition duration-700 group-hover:scale-[1.045] group-hover:opacity-68 group-hover:grayscale-0" /><div className={`absolute inset-0 bg-gradient-to-br ${item.gradient} opacity-42 transition duration-500 group-hover:opacity-58`} /><div className="absolute inset-0 bg-gradient-to-t from-black via-black/25 to-transparent" /><div className="absolute -right-12 -top-12 h-44 w-44 rounded-full bg-white/6 blur-2xl transition group-hover:bg-zephyr-cyan/12" /><div className="relative z-10 flex h-full min-h-[390px] flex-col justify-between p-6 md:min-h-[470px] md:p-7"><span className="text-6xl font-black text-white/16 md:text-7xl">{item.num}</span><div><h3 className="text-2xl font-black uppercase tracking-[-0.04em] text-white transition group-hover:text-zephyr-cyan">{item.title}</h3><p className="mt-4 max-w-xs text-sm leading-6 text-white/68">{item.copy}</p><span className="mt-7 inline-flex text-xs font-bold uppercase tracking-[0.22em] text-zephyr-magenta">Fullscreen Preview →</span><span className="mt-2 block text-[0.58rem] uppercase tracking-[0.18em] text-white/34">Temporary {item.source}</span></div></div></motion.button></Reveal>)}</div><AnimatePresence>{active && <motion.div role="dialog" aria-modal="true" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[80] flex items-center justify-center bg-black/88 p-4 backdrop-blur-md" onClick={() => setActive(null)}><motion.div initial={{ scale: .92, y: 40 }} animate={{ scale: 1, y: 0 }} exit={{ scale: .94, y: 30 }} transition={{ type: 'spring', damping: 24, stiffness: 180 }} className="relative h-[86vh] w-full max-w-6xl overflow-hidden rounded-[2rem] border border-white/15 shadow-[0_30px_120px_rgba(0,0,0,.55)]" onClick={(e) => e.stopPropagation()}><img src={active.image} alt="" className="absolute inset-0 h-full w-full object-cover opacity-75" /><div className={`absolute inset-0 bg-gradient-to-br ${active.gradient}`} /><div className="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent" /><button type="button" onClick={() => setActive(null)} className="absolute right-4 top-4 rounded-full border border-white/20 bg-black/40 px-4 py-2 text-xs font-bold uppercase tracking-[0.18em] backdrop-blur-md hover:border-zephyr-cyan hover:text-zephyr-cyan">Close</button><div className="absolute bottom-6 left-6 right-6 md:bottom-10 md:left-10"><p className="text-sm uppercase tracking-[0.34em] text-zephyr-cyan">{active.num} / Cinematic Preview</p><h3 className="mt-3 text-5xl font-black uppercase leading-none tracking-[-0.07em] md:text-8xl">{active.title}</h3><p className="mt-5 max-w-2xl text-base leading-7 text-white/72 md:text-xl">{active.copy}</p><p className="mt-4 text-xs uppercase tracking-[0.25em] text-white/42">Temporary stock-style placeholder — ready to swap with original Zephyr AI assets.</p></div></motion.div></motion.div>}</AnimatePresence></section>;
}

function Services() {
  return <section id="services" className="px-4 py-28 md:px-6 md:py-36"><SectionTitle kicker="Services" title="Frames first. Words second." /><div className="mx-auto grid max-w-7xl gap-4 sm:grid-cols-2 lg:grid-cols-4">{services.map((s, i) => <Reveal delay={i*.03} key={s}><motion.div whileHover={{ y: -5 }} className="glass min-h-44 rounded-[1.5rem] p-6 transition hover:border-zephyr-cyan/25 hover:shadow-[0_18px_60px_rgba(0,0,0,.22)] md:min-h-52 md:rounded-[1.8rem] md:p-7"><div className="mb-8 h-10 w-10 rounded-full bg-gradient-to-br from-zephyr-cyan/55 to-zephyr-magenta/45 shadow-[0_0_18px_rgba(123,223,229,.10)]" /><h3 className="text-lg font-bold uppercase tracking-[-0.03em]">{s}</h3></motion.div></Reveal>)}</div></section>;
}
...
function Pricing() {
  return <section id="pricing" className="bg-[#05050c] px-4 py-28 md:px-6 md:py-36"><SectionTitle kicker="Pricing" title="Premium campaign architecture" copy="Three ways to turn a brief into cinematic attention." /><div className="mx-auto grid max-w-7xl gap-5 lg:grid-cols-3">{prices.map(([name, price, copy, features], i) => <Reveal delay={i*.06} key={name}><div className={`glass h-full rounded-[1.6rem] p-7 md:rounded-[2rem] md:p-8 ${i===2 ? 'border-zephyr-magenta/28 shadow-magenta' : ''}`}><p className="text-xs uppercase tracking-[0.3em] text-zephyr-cyan">{name}</p><h3 className="magenta-glow mt-6 text-4xl font-black">{price}</h3><p className="mt-5 text-sm leading-7 text-white/64 md:text-base">{copy}</p><ul className="mt-8 space-y-3 text-sm text-white/72">{features.map(f => <li key={f} className="flex gap-3"><span className="text-zephyr-cyan" aria-hidden="true">✦</span><span>{f}</span></li>)}</ul></div></Reveal>)}</div></section>;
}
